...
`~/Desktop/dev/documentation` (rendered to https://shinylib.net/speech).

Shiny.Speech is a cross-platform speech-to-text / text-to-speech and audio capture/playback library
with pluggable cloud providers (Azure, OpenAI, ElevenLabs, Microsoft.Extensions.AI) plus native
...
- Feature pages: `src/content/docs/speech/*.mdx` (e.g. `architecture.mdx`, `azure.mdx`,
  `openai.mdx`, `elevenlabs.mdx`, `microsoft-ai.mdx`, `custom-provider.mdx`).
- Release notes: `src/content/docs/speech/release-notes.mdx`.
...
2. **Skill** (`skills/`) — the agent-facing "how to generate correct code" doc; update the trigger
   keyword list when a new provider / public API is introduced.
...
`type="feature|enhancement|fix|chore"` and an optional `breaking` flag. Group under a `## v2`-style
version heading; newest version section stays at the top. Use a `### <version> - TBD` heading for
unreleased work and promote it to a dated heading (`### 2.1 - May 21, 2026`) when cutting the
release.
...
- Frontmatter:
  ...
  ---
  ```
- Body is MDX. Reuse components where relevant, e.g. `import NugetBadge from '/src/components/NugetBadge.astro';` then `<NugetBadge name="Shiny.Speech" />`.
